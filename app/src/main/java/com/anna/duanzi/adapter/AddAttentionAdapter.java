 ...
 */
public class AddAttentionAdapter extends
        TflListAdapter<AVUser> {
    private Context mContext;

    public AddAttentionAdapter(List<AVUser> data, Context context) {
        super(data);
    ...
    public RecyclerView.ViewHolder onCreateDataViewHolder(ViewGroup parent) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.add_attention_item, parent, false);
    ...
    public void onBindDataViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        ...
        Button button = ((ViewHolder) holder).btn_add;
        button.setText("+关注");
        button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                addAttention(user.getObjectId(), position);
            }
        });
        holder.itemView.setTag(user);
    }

    private void addAttention(String userObjectId, final int position) {
