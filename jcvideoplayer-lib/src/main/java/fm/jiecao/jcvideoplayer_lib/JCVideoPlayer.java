...
public class JCVideoPlayer extends FrameLayout implements View.OnClickListener, SeekBar.OnSeekBarChangeListener, SurfaceHolder.Callback, View.OnTouchListener {
    ...
    public static long clickfullscreentime;
...
//        surfaceView.setBackgroundColor(R.color.black_a10_color);
    ...
    public void setUpForVideoActivity(String url, String thumb) {
        this.url = url;
...
//            surfaceView.setBackgroundColor(R.color.black_a10_color);
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start || i == R.id.thumb) {
            ...
            if (ifFullScreen) {
                ...
                VideoEvents videoEvents = new VideoEvents().setType(VideoEvents.VE_SURFACEHOLDER_FINISH_VIDDEOACTIVITY);
                EventBus.getDefault().post(videoEvents);
            }
        }
    }
    ...
    private void setTitleVisibility(int visable) {
        if (ifShowTitle) {
            ...
            if (ifFullScreen || isVideoActivity) {
                llTitleContainer.setVisibility(visable);
            } else {
    ...
    private void setProgressAndTime(int progress, int currentTime, int totalTime) {
        ...
        int processTime = Utils.getProcessTime(currentTime);
        if (!JCMediaManager.intance().isVip && processTime > JCMediaManager.intance().NO_VIP_FREE_TIME) {
            if (ifFullScreen) {
                //如果是全屏就先退出全屏
                quitFullScreen();
            }
            EventBus.getDefault().post( new VideoEvents().setType(VideoEvents.VE_IS_VIP));
        }
...
//        cancelDismissControlViewTimer();
    ...
    public void sendPointEvent(int type) {
